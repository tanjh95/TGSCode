...
void sg4Detector::ConstructSDandField()
{
  ...
  G4bool forAllDaughters=true;
  logicmcp->SetFieldManager(pFMng, forAllDaughters);
...
//      logicVoxel[j]=new G4LogicalVolume(solidVoxel,vacuum,str1);
      	...
      	if(j==4){
		      logicVoxel[j]=new G4LogicalVolume(solidVoxel,SiO2,str1);
	        }
...
if(par.Energycut==1){    
  ...
  //Livermore250eV，DNA<10eV,100MeV,DNA尚未调出来
...
// G4Region *fRegion = new G4Region("MCP");
...
//  fRegion->SetProductionCuts(cuts);
  //fRegion->AddRootLogicalVolume(logicmcp); 
  //fRegion->AddRootLogicalVolume(logicChannel); 
}
